...
AnimationPlayer::AnimationPlayer(Animation *data) {
	setAnimData(data);
}
...
bool AnimationPlayer::setAnimData(Animation* _data) {
...
vec4i AnimationPlayer::getCurrentFrame() const {
	assert(data != NULL);
...
int AnimationPlayer::getAnimID(std::string name) {
...
int AnimationPlayer::getLoopsLeft() const {
	return loopsLeft;
}
...
bool Animation::saveToFile(std::string filename) {
	...
	for (unsigned int i = 0; i < animations.size(); ++i) {
		...
		for (unsigned int j = 0; j < track.frames.size(); ++j) {
			...
			out << frame.rect.x <<" "<< frame.rect.y <<" "
				<< frame.rect.z <<" "<< frame.rect.w <<" "
				<< frame.time << std::endl;
		}
	}
	return true;
...
std::set<std::string> Animation::getContentFilename() {
...
void Animation::getContentFilename(std::set<std::string> &contentFilename)  {
	std::vector<AnimationTrack>::iterator it = animations.begin();
	for(;it != animations.end(); ++it) {
		contentFilename.insert(it->spritesheetFilename);
	}
